// api/auth/reset.js - API récupération de compte
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Envoyer le code
    if (method === 'email') {
      const emailResult = await sendRecoveryEmail(user.email, code, user.fullName);
    ...
    return res.status(200).json({
      ...
      message: `Code de récupération envoyé par ${method}`
    });
